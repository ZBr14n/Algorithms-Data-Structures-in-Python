...
# =============================================================================
nums = [8,1,1,2]  #should output 9 in the end
...
def findMinimumStrokes(inputArray):
    ...
    for j in range(findTallestHeight(inputArray)):   
        isTallerColumn = False
        for k in range(len(inputArray)):             
            #compare current column and next column
            if not isTallerColumn and inputArray[k] >= (j+1):
                countStrokes+=1
                isTallerColumn = True
            else:
                if isTallerColumn and inputArray[k] < (j+1):
                    isTallerColumn = False
